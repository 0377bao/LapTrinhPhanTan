...
public class MapGui extends JFrame {
    ...
    private GiaoHangGui guiGH;
    private MapGui frame;

    public MapGui(GiaoHangGui gh) {
        frame = this;
        guiGH = gh;
        ...
        MyWayPoint wayPoint = new MyWayPoint(new GeoPosition(10.822065, 106.686810), MyWayPoint.PointType.START, even,
                "Nơi giao hàng");
        ...
        btnNewButton_1.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (textField.getText().equals("")) {
                    JOptionPane.showMessageDialog(MapGui.this, "Vui lòng nhập địa chỉ nhận hàng");
                } else {
                    GeoPosition geop = Jxmapviewer.convertPointToGeoPosition(mousePoint);
                    MyWayPoint wayPoint = new MyWayPoint(new GeoPosition(geop.getLatitude(), geop.getLongitude()),
                            MyWayPoint.PointType.END, even, "Nơi nhận hàng");
                    addWaypoint(wayPoint);
        ...
        txtHi.setText(
                "Trường Đại học Công nghiệp TP.HCM, 12 Nguyễn Văn Bảo, Phường 4, Gò Vấp, Thành phố Hồ Chí Minh, Việt Nam");
        ...
        btnTim.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (diaChi.equals("")) {
                    ...
                    if (toaDo.size() == 0) {
                        JOptionPane.showMessageDialog(MapGui.this, "Địa chỉ nhận hàng không hợp lệ");
                    } else {
                        MyWayPoint wayPoint = new MyWayPoint(new GeoPosition(toaDo.get(0), toaDo.get(1)),
                                MyWayPoint.PointType.END, even, "Nơi nhận hàng");
                        addWaypoint(wayPoint);
            ...
            public void actionPerformed(ActionEvent e) {
                if (textField_1.getText().equals("")) {
                    JOptionPane.showMessageDialog(frame,
                            "Vui lòng nhập địa chỉ nhận hàng và chọn chức năng tìm trước khi hoàn tất");
                    return;
                } else {
    ...
    public void initWaypoint() {
        ...
        if (myWaypoints.size() == 2) {
            ...
            if (start != null && end != null) {
                resultRoutig = RoutingService.getInstance().routing(start.getLatitude(), start.getLongitude(),
                        end.getLatitude(), end.getLongitude());
                if (resultRoutig.size() == 0) {
                    JOptionPane.showMessageDialog(MapGui.this,
                            "Nơi nhận hàng không hợp lệ hoặc nằm ngoài phạm vi lãnh thổ Việt Nam");
    ...
    public EvenWaypoint getEven() {
        return new EvenWaypoint() {
            ...
            public void selected(MyWayPoint wayPoint) {
                JOptionPane.showMessageDialog(MapGui.this, wayPoint.getName());
            }
        };
    }
